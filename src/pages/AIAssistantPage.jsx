  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-8 space-y-6 sm:space-y-8 transition-colors duration-300">
      ...
      <div className="space-y-2 text-center max-w-2xl mx-auto">
        ...
        <h1 className="text-2xl sm:text-3xl font-black text-[var(--ox-text-primary)]">AI Resume Assistant Suite</h1>
      ...
      {isGuest && (
        <div className="p-6 rounded-2xl bg-gradient-to-r from-amber-500/20 via-orange-500/10 to-[var(--ox-surface-secondary)] border border-orange-500/40 text-center space-y-4 shadow-xl transition-colors duration-300">
          ...
          <button
            ...
            className="min-h-[44px] px-6 py-3 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-extrabold text-xs rounded-xl shadow-lg transition-all inline-flex items-center gap-2 cursor-pointer active:scale-95"
          >
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-[var(--ox-text-primary)]">{feat.name}</span>
                  <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-lg bg-orange-500/20 text-orange-400 border border-orange-500/30">
                    {feat.requiredCredits} Credit{feat.requiredCredits > 1 ? 's' : ''}
                  </span>
                </div>
                <p className="text-[11px] text-[var(--ox-text-secondary)]">{feat.description}</p>
              </div>
            );
          })}
        </div>
      ...
      <div className="p-5 sm:p-6 rounded-2xl bg-[var(--ox-surface-secondary)] border border-[var(--ox-border)] space-y-6 max-w-4xl mx-auto shadow-lg">
        ...
        {/* Cost Estimator Box */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-3.5 rounded-xl bg-[var(--ox-surface-primary)] border border-[var(--ox-border)] text-xs">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {['Professional', 'Friendly', 'Corporate', 'Technical'].map((t) => (
              <button
                ...
                type="button"
                onClick={() => setTone(t)}
                className={`min-h-[40px] py-1.5 rounded-xl text-xs font-bold border transition-all cursor-pointer ${
                  tone === t
        ...
        <button
          ...
          className="min-h-[44px] px-5 py-2.5 text-xs font-extrabold text-black bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-400 hover:to-amber-400 rounded-xl shadow-[0_0_18px_rgba(249,115,22,0.3)] transition-all flex items-center gap-2 cursor-pointer active:scale-95 disabled:opacity-50"
        >
          {isExecuting ? (
            <>
