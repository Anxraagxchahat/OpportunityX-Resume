  ...
  return (
    <header className="w-full max-w-full bg-[var(--ox-surface-primary)] border-b border-[var(--ox-border)] sticky top-0 z-30 transition-colors duration-200 no-print select-none box-border">
      ...
      <div className="px-3 py-2 flex items-center justify-between gap-2 w-full max-w-full box-border">
        ...
        {/* Right: Theme Toggle, Credits & Auth Profile Badges */}
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button
            ...
            className="px-2.5 py-1.5 rounded-xl bg-orange-500/10 hover:bg-orange-500/20 border border-orange-500/30 text-orange-400 text-xs font-bold flex items-center gap-1 min-h-[44px] cursor-pointer active:scale-95 transition-transform shrink-0"
            title="AI Credits"
            aria-label="AI Credits Balance"
          >
            <Sparkles className="w-3.5 h-3.5 animate-pulse text-amber-400 shrink-0" />
            <span className="truncate">{session.isAuthenticated && !session.isGuest ? `${aiCredits.remaining} Cr` : '5 Cr'}</span>
          </button>

          {/* Profile / Login Account Button */}
          <button
