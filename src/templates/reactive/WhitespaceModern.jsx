 ...
 */
export const WhitespaceModernTemplate = ({ resumeData, accentHex, fontFamily, visibleBlockIds = null }) => {
  ...
  return (
    <div className="text-slate-800" style={{ fontFamily: `'${fontFamily || 'Inter'}', sans-serif` }}>
      {/* Airy Header */}
      {isVisible('header') && (
        <div data-block-id="header" className="pb-5 mb-5 pdf-block pdf-keep-together">
        ...
        {experience.length > 0 && experience.some((_, i) => isVisible(`exp-${i}`)) && (
          <div>
            ...
            {experience.map((exp, i) => {
              ...
              return (
                <div key={exp.id || i} data-block-id={blockId} className="mb-4 last:mb-0 pdf-block pdf-item">
                  ...
                  {exp.bullets && (
                    <ul className="mt-1.5 space-y-1 text-[10px] text-slate-600">
                      {exp.bullets.map((b, idx) => <li key={idx} className="pl-3 relative before:content-['–'] before:absolute before:left-0 before:text-slate-400">{b}</li>)}
                    </ul>
                  )}
                </div>
              );
