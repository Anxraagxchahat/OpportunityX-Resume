...
export const ExperienceBlock = ({ experience, accentHex, variant = 'default', visibleBlockIds = null }) => {
  ...
  if (variant === 'compact') {
    return experience.map((exp, i) => {
      ...
      return (
        <div key={exp.id || i} data-block-id={blockId} className="mb-2 last:mb-0 pdf-block pdf-item">
  ...
  if (variant === 'inline-header') {
    return experience.map((exp, i) => {
      ...
      if (!isVisible(blockId)) return null;
      return (
        <div key={exp.id || i} data-block-id={blockId} className="mb-2.5 last:mb-0 pdf-block pdf-item">
  ...
  return experience.map((exp, i) => {
    ...
    if (!isVisible(blockId)) return null;
    return (
      <div key={exp.id || i} data-block-id={blockId} className="mb-3 last:mb-0 pdf-block pdf-item">
        <div className="flex justify-between items-baseline text-xs">
