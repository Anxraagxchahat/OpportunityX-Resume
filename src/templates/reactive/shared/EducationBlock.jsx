...
export const EducationBlock = ({ education, accentHex, variant = 'default', visibleBlockIds = null }) => {
  ...
  if (variant === 'compact') {
    return education.map((edu, i) => {
      ...
      return (
        <div key={edu.id || i} data-block-id={blockId} className="mb-2 last:mb-0 pdf-block pdf-item">
  ...
  return education.map((edu, i) => {
    ...
    if (!isVisible(blockId)) return null;
    return (
      <div key={edu.id || i} data-block-id={blockId} className="mb-2.5 last:mb-0 pdf-block pdf-item">
